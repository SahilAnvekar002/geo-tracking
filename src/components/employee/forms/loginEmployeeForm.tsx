...
import { loginEmployeeSchema } from "@/lib/schema/employee.schema";
import Image from "next/image";
import { zodResolver } from "@hookform/resolvers/zod";
import { getEmployeeByEmail } from "@/actions/employee.actions";
import { useRouter } from "next/navigation";

export default function LoginEmployeeForm() {
  const router = useRouter();

  const form = useForm<z.input<typeof loginEmployeeSchema>>({
  ...
  const onSubmit = async (values: z.input<typeof loginEmployeeSchema>) => {
    setLoading(true);

    const response = await getEmployeeByEmail(values.email);
    ...
    const res = await fetch("/api/employee/login", {
      ...
      body: JSON.stringify({ id: response.data }),
    });
      ...
      <Card className="">
        <div className="py-4 px-4">
          <CardHeader className="flex flex-col items-center mb-6 text-center">
            ...
            />
            <CardTitle className="text-xl">Welcome Back - Employee Login</CardTitle>
          </CardHeader>
          <CardContent>
